...
var MUSIC = (function() {
    ...
    return {
        ...
        update: function() {
            ...
            tracks.sopr.element.volume = VOLUME_FACTOR * Math.max(0, Math.min(1, separation));
            ...
            tracks.drum.element.volume = tracks.atmo.element.volume
                                       = VOLUME_FACTOR * Math.max(0, Math.min(1, mortality));
            ...
            var speed = Math.sqrt(p(1).xVel * p(1).xVel + pspr(1)._gy * pspr(1)._gy) +
                ...
                haste = speed / 50 + 0.5;
            tracks.alto.element.volume = VOLUME_FACTOR * Math.max(0, Math.min(1, haste));
        }
    };
})();
